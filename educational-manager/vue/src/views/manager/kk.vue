<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入申请说明" style="width: 200px" v-model="content"></el-input>
...
<!--      <el-button type="primary" plain @click="handleAdd">队伍申请</el-button>-->
<!--    </div>-->
...
<!--            <el-button plain type="primary" @click="handleEdit(scope.row)" size="mini" v-if="user.role === 'STUDENT' && scope.row.status !== '审核通过'">编辑</el-button>-->
            <el-button plain type="primary" @click="handleCheck(scope.row)" size="mini" :disabled="scope.row.status !== '待审核'">审核</el-button>
...
export default {
  ...
  methods: {
    ...
    load(pageNum) {  // 分页查询
      ...
      this.tableData = this.mockedTableData.slice(start, end);
      this.total = this.mockedTableData.length;
    },
    reset() {
      this.status = null
      this.content = null
